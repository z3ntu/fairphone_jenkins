...
git -c "user.name=$username" -c "user.email=$useremail" -C $repopath reset $(git -c "user.name=$username" -c "user.email=$useremail" -C $repopath commit-tree HEAD^{tree} -m "MultiROM FP2")
