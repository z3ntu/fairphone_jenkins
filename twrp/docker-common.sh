...
function start_docker() {
  ...
  sudo docker run --rm --net=host $MOUNTS $DOCKER_IMAGE /bin/bash -c "if [ -d fairphone_jenkins/ ]; then git -C fairphone_jenkins/ pull; else git clone https://github.com/z3ntu/fairphone_jenkins; fi; ./fairphone_jenkins/twrp/$ACTION.sh"
}
